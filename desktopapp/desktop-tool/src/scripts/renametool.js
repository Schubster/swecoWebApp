const fs = require('fs');
    ...
    fetchData["standard_id"] = projectID;
        ...
        ipcRenderer.send("apiRequest", fetchData, "http://127.0.0.1:8000/api/fetchAllProjectStandards", "standardDictsResponse");

        ipcRenderer.on("standardDictsResponse", (event, responseData) => {
            const [{ standards_by_type: standards }] = JSON.parse(responseData);
